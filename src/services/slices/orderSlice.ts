...
export const fetchOrderByNumber = createAsyncThunk<
  TOrder,
  number
>('order/fetchByNumber', async (orderNumber, { rejectWithValue }) => {
  try {
...
const orderSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(placeOrder.fulfilled, (state, action: PayloadAction<TOrder>) => {
      state.orderRequest = false;
    ...
    builder.addCase(fetchOrderByNumber.fulfilled, (state, action: PayloadAction<TOrder>) => {
      state.orderRequest = false;
      state.orderData = action.payload;
    });
